...
def get_image_row(images, interpolation=3, height="min"):
    ...
    if type(height) is str:
        ...
        raise TypeError("height should be a int or a string (numpy attribute"
                        "like min/max/median)")

    imgs_comn = np.hstack([img_.resize(np.round(
        np.asarray(img_.size)*(height / img_.size[1])).astype("int"),
                                       interpolation)
                           for img_ in images])
...
def get_image_column(images, interpolation=3, width="min"):
    ...
    if type(width) is str:
        ...
        raise TypeError("width should be a int or a string (numpy attribute"
                        "like min/max/median)")

    imgs_comn = np.vstack([img_.resize(np.round(
        (np.asarray(img_.size)*(width / img_.size[0]))).astype("int"),
                                        interpolation)
                           for img_ in images])
    return Image.fromarray(imgs_comn)
...
def get_buffer(size, text="", hline=None, vline=None, fontsize='medium',
    ...
    """ Uses matplotlib to create a simple full size axis in which you can
    add text and horizonthal lines """
