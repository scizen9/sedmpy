...
class SedmDB:
    class __SedmDB:
        ...
        def __getSedmDBConn__(self):
            ...
            """
            sedmdbcon = psycopg2.connect(host=self.host, port="5432",
                                         dbname=self.dbname,
    ...
    def add_user(self, pardic):
        ...
        usernames = [user[0] for user in self.execute_sql(
        ...
        try:
            ...
            return -1, "ERROR: add_user sql command failed with an " \
                       "IntegrityError!"
        except exc.ProgrammingError:
            return -1, "ERROR: add_user sql command failed with a " \
                       "ProgrammingError!"
        return tid, "User added"
    ...
    def update_user(self, pardic):
        ...
        Returns:
            ...
            (id (long), "User updated, columns 'column_names'") if the user is
        ...
        for key in reversed(keys):  # remove any keys that are invalid or
        ...
        try:
            ...
            return -1, "ERROR: update_user sql command failed with an " \
                       "IntegrityError!"
        except exc.ProgrammingError:
            return -1, "ERROR: update_user sql command failed with a " \
                       "ProgrammingError!"
    ...
    def remove_user(self, pardic):
        ...
        Returns:
            (-1, "ERROR...") if there was an issue (user doesn't exist,
            not enough information in pardic)
    ...
    def get_from_users(self, values, where_dict={}, compare_dict={}):
        ...
        """
        allowed_params = {'id': int, 'username': str, 'name': str,
    ...
    def add_group(self, pardic):
        ...
        Returns:
            (-1, "ERROR...") if no designator was provided or there is already
             a group with it
        ...
        groups = [des[0] for des in self.execute_sql(
            'SELECT designator FROM groups;')]
        if pardic['designator'] not in groups:
            ...
            try:
                ...
                return -1, "ERROR: add_group sql command failed with an " \
                           "IntegrityError!"
            except exc.ProgrammingError:
                return -1, "ERROR: add_group sql command failed with a " \
                           "ProgrammingError!"
    ...
    def add_usergroup(self, user, group):
        ...
        usergroups = self.execute_sql('SELECT user_id, '
                                      'group_id FROM usergroups')
    ...
    def remove_from_group(self, user, group):
        ...
        if (user, group) not in usergroups:
            ...
            sql = "DELETE FROM usergroups WHERE user_id='%s' AND group_id='%s'" \
                  % (user, group)
            try:
                self.execute_sql(sql)
            except exc.IntegrityError:
    ...
    def get_from_usergroups(self, values, where_dict={}, compare_dict={}):
        ...
        Returns:
            list of tuples containing the values for each usergroup matching
            the criteria
    ...
    def add_program(self, pardic):
        ...
        if 'designator' in keys:
            if pardic['designator'] in [obj[0] for obj in
                ...
                return -1, "ERROR: a program with that designator already " \
                           "exists!"
    ...
    def update_program(self, pardic):
        ...
        elif pardic['id'] not in [x[0] for x in
        ...
        try:
            ...
            return -1, "ERROR: update_program sql command failed with an " \
                       "IntegrityError!"
        except exc.ProgrammingError:
            return -1, "ERROR: update_program sql command failed with a " \
                       "ProgrammingError!"
    ...
    def get_from_program(self, values, where_dict={}, compare_dict={}):
        ...
        Args:
            ...
            values/keys options: ['id', 'designator', 'name', 'group_id', 'PI',
             'time_allocated', 'priority', 'inidate', 'enddate', 'color']

        Returns:
            list of tuples containing the values for each program matching
            the criteria
    ...
    def add_allocation(self, pardic):
        ...
        Args:
            pardic (dict):
                ...
                optional:
                    'time_allocated' (datetime.timedelta object or float/int
                    seconds)
                    'time_spent' (datetime.timedelta object or float/int
                    seconds)
                    'inidate' ('year-month-day hour:minute:second')
                    'enddate' ('year-month-day hour:minute:second')
                    'color' (hex color code)
        ...
        for key in reversed(keys):  # remove any extraneous keys
            if key not in ['id', 'program_id', 'designator', 'time_allocated',
                           'time_spent', 'inidate', 'enddate', 'color',
                           'active']:
                return -1, "ERROR: %s is an invalid key!" % (key,)
        type_check = _data_type_check(keys, pardic, param_types)
        if type_check:
        ...
        try:
            ...
            return -1, "ERROR: add_program sql command failed with an " \
                       "IntegrityError!"
        except exc.ProgrammingError:
            return -1, "ERROR: add_program sql command failed with a " \
                       "ProgrammingError!"
    ...
    def update_allocation(self, pardic):
        ...
        Returns:
            ...
            (id (long), "Allocation updated, columns 'column_names'") if the
            allocation is updated successfully
        """
    ...
    def get_from_allocation(self, values, where_dict={}, compare_dict={}):
        ...
        Args:
            ...
            compare_dict (dict): default is {}
                'param': 'inequality' (i.e. '>', '<', '>=', '<=', '<>', '!='))
                if no inequality is provided, '=' is assumed
            values/keys options: ['id', 'designator', 'program_id, 'time_spent', 'time_allocated',
             'inidate', 'enddate', 'color', 'active']
    ...
    def add_object(self, pardic):
        ...
        Args:
            pardic (dict):
                ...
                'typedesig' should be one of:
                    'f' (fixed), 'v' (periodic fixed), 'P' (built-in planet or satellite name),
                    'e' (heliocentric elliptical), 'h' (heliocentric hyperbolic), 'p' (heliocentric parabolic),
        ...
        pardic['name'] = pardic['name'].lower()  # make all the names the same format for consistant searching
        if pardic['typedesig'] == 'f':
            ...
            try:
                ...
                return -1, "ERROR: add_object sql command failed with a ProgrammingError!"
            return tid, "Fixed object added, input period data into table `periodic`"
    ...
    def add_hyperbolic_heliocentric(self, orbit_params):
        ...
        """
        param_types = {'id': int, 'object_id': int, 'T': 'date', 'e': float, 'inclination': float,
                       'longascnode_O': float,
        ...
        return tid, "Hyperbolic heliocentric orbit added"
    ...
    def add_parabolic_heliocentric(self, orbit_params):
        ...
        return tid, "Parabolic heliocentric orbit added"
    ...
    def add_earth_satellite(self, orbit_params):
        ...
        return tid, "Earth satellite orbit added"
    ...
    def update_request(self, pardic):
        ...
        elif not (isinstance(pardic['id'], int) or isinstance(pardic['id'], int)):
    ...
    def get_from_observation(self, values, where_dict={}, compare_dict={}):
        ...
        Args:
            ...
            values/keys options:
                ...
                'camera' (str),
                'time_elapsed (datetime.timedelta object or float/int seconds),
                'filter' (str) 'u', 'g', 'r', 'i', 'ifu', 'ifu_a', 'ifu_b'
    ...
    def add_telescope_stats(self, tel_stats):
        ...
        """
        telstat_types = {'id': int, 'date': 'date', 'dome_status': str, 'in_temp': float, 'in_humidity': float,
                         'in_dew': float,
    ...
    def get_from_telescope_stats(self, values, where_dict={}, compare_dict={}):
        ...
        sql = _generate_select_sql(values, where_dict, allowed_params, compare_dict,
                                   'telescope_stats')  # checks type and
        if sql[0] == 'E':  # if the sql generation returned an error
            return -1, sql
    ...
    def add_phot(self, pardic):
        ...
        """
        param_types = {'id': int, 'observation_id': int, 'astrometry': 'bool', 'filter': str, 'reducedfile': str,
                       'sexfile': str,
    ...
    def get_from_phot(self, values, where_dict={}, compare_dict={}):
        ...
        Returns:
            ...
            (-1, "ERROR...") if there was an issue
        """
        allowed_params = {'observation_id': int, 'astrometry': 'bool', 'filter': str, 'reducedfile': str,
                          'sexfile': str,
    ...
    def add_metrics_phot(self, pardic):
        ...
        return tid, "Photometry metrics added"
    ...
    def get_from_metrics_phot(self, values, where_dict):
        ...
        sql = _generate_select_sql(values, where_dict, allowed_params,
                                   table='metrics_phot')  # checks type and
        if sql[0] == 'E':  # if the sql generation returned an error
            return -1, sql
    ...
    def add_phot_calib(self, pardic):
        ...
        return tid, "Photometry calibration added"
    ...
    def add_spec_calib(self, pardic):
        ...
        return tid, "Spectrum calibration added"
    ...
    def add_classification(self, pardic):
        ...
        return tid, "Classification added"
    ...
    def update_classification(self, pardic):
        ...
        if 'id' in keys:
            ...
            tid = self.get_from_classification(['id'], {'spec_id': pardic['spec_id'],
    ...
    for key in keys:
        if value_types[key] == str:
            ...
            try:
                ...
                return "ERROR: %s must be %s!" % (key, str(int)[1:-1])
