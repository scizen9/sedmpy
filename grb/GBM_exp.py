"""
   ...
   For a given magnitude and time of a GRB, calculate the expected magnitude at
   a later time assuming a power law decay.
...
# delta_t = float(inputs[3])  # delta_t from t0 to time of obs with SEDm [s]
...
t = np.linspace(t0, tend, 1000)                  # Range of observe times to [s]
m = 2.5 * gamma * np.log10(t / t0) + m0          # final magnitude
m_exp = 2.5 * gamma * np.log10(t_obs / t0) + m0  # mag at t_obs = t0+delta_t
print("Expected Magnitude %s s after initial UVOT observations: %.2f" %
      (delta_t, m_exp))
...
p2 = plt.scatter(t0+delta_t, m_exp, c='r',
                 label="Expected Magnitude at Observation")
plt.legend()
