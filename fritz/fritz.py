...
fritz_inst_url = fritz_base_url + 'update_followup_config.cgi'
fritz_stat_url = fritz_base_url + 'update_followup_status.cgi'
fritz_spec_url = fritz_base_url + 'add_spec_auto.cgi'
fritz_phot_url = fritz_base_url + 'edit_phot_auto.cgi'
fritz_view_source_url = fritz_base_url + 'view_source.cgi?'
...
def send_instrument_configuration(instrument_id="",
    ...
    available options shown on the fritz marshal page
    ...
    # 4. Send json file request to fritz marshal
    ret = requests.post(fritz_inst_url,
                        auth=(user, pwd),
...
def update_target_by_object(objname, add_spectra=False, spectra_file='',
    ...
    if search_db:
        ...
        # dictionary from fritz.
        if pull_requests:
            print("Gathering all request files")
            import fritz.fritz_watcher as fritz_watcher
            fritz_watcher.pull_request_from_remote()
        match_list = []
    ...
    if marshal_id <= 0:
        ...
        return_link = fritz_view_source_url + "name=%s" % object_name
