...
def get_flats_counts(directory):
    """
    Reads all the images in a directory marked as "twilight" and uses them to
    infer the average count rate for a given number of seconds after the sunset.

    It plots the number of counts and fits a 2D polynomial used to interpolate
    in the future.
    ...
    for f in glob.glob(directory + "/rc*fits"):
        if fitsutils.has_par(f, "IMGTYPE") and \
                fitsutils.get_par(f, "IMGTYPE") == "TWILIGHT":
            flatlist.append(f)
    ...
    for f in flatlist:
        ...
        elapsed = 3600 * (utc.hour - st.hour) + 60 * (
                    utc.minute - st.minute) + (utc.second - st.second)
    ...
    t = Table(names=('filter', 'c2', 'c1', 'c0'),
              dtype=('S1', 'f8', 'f8', 'f8'))

    for band in corners.keys():
        print(sun_decs[band], np.log10(counts[band]))
        coefs = np.polyfit(sun_decs[band], np.log10(counts[band]),
                           deg=2)  # , w=1./np.sqrt(np.log10(counts[band])))
        p = np.poly1d(coefs)
...
def get_flat_itime(band, elapsed, counts=20000):
    """
    Provides the optimum integration time to achieve the desired number of
    counts. Its parameters are the filter in which we are observing, and the
