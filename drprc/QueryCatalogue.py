...
class QueryCatalogue:
    ...
    def query_usnob1(self):
        ...
        # ra, dec = coordinates_conversor.hour2deg(f[0].header['RA'],
        # f[0].header['DEC'])
        # SEDM FoV is 6.5 arcmin, due to uncertainties in the position,
        # 4 arcmin radius assumed.
        # Download USNO-B1 catalog for the position
    
        timestamp = datetime.datetime.isoformat(datetime.datetime.utcnow())

        catalog_url = 'http://www.nofs.navy.mil/cgi-bin/vo_cone.cgi?' \
                      'CAT=USNO-B1&RA=%.5f&DEC=%.5f&SR=%.4f&VERB=1' % \
    ...
    def query_apass(self):
        ...
        catalog_url = 'http://www.aavso.org/cgi-bin/apass_download.pl?' \
                      'ra=%.5f&dec=%.5f&radius=%.4f8&outtype=1' % \
                      (self.ra, self.dec, self.rad)
    ...
    def query_sdss(self):
        ...
        catalog_url = 'http://skyserver.sdss.org/dr9/en/tools/search/' \
                      'x_radial.asp?ra=%.5f&dec=%.5f&check_type=type&type=6&' \
                      'radius=%.4f&check_u=u&min_u=%.2f&max_u=%.2f&check_g=g&' \
                      'min_g=%.2f&max_g=%.2f&check_r=r&min_r=%.2f&max_r=%.2f&' \
                      'check_i=i&min_i=%.2f&max_i=%.2f&check_z=z&min_z=%.2f&' \
                      'max_z=%.2f&entries=top&topnum=500&format=csv' % \
                      (self.ra, self.dec, self.rad*60, self.minmag, self.maxmag,
                       self.minmag, self.maxmag, self.minmag, self.maxmag,
                       self.minmag,self.maxmag,self.minmag,self.maxmag)
        ...
        if len(catalog.dtype) == 1:
            catalog = np.array([], dtype=[('objid', '<f8'), ('run', '<f8'),
                                          ('rerun', '<f8'), ('camcol', '<f8'),
                                          ('field', '<f8'), ('obj', '<f8'),
                                          ('type', '<f8'), ('ra', '<f8'),
                                          ('dec', '<f8'), ('u', '<f8'),
    ...
    def query_catalogue(self, catalogue="PS1V3OBJECTS", filtered=True):
        ...
        General Catalog Access : http://gsss.stsci.edu/webservices/vo/
        CatalogSearch.aspx?Parameters...
        ...
        Optional Parameters
            ...
            FORMAT= VOTABLE(default) | HTML | KML | CSV | TSV | JSON |
            TEXT(limited set of catalogs)
            CATALOG=GSC23(default) | GSC11 | GSC12 | USNOB | SDSS | FIRST |
            2MASS | IRAS | GALEX | GAIA | TGAS | WISE
        ...
        if catalogue == "PS1V3OBJECTS":
            ...
            if filtered:
                # Filter spurious sources / Objects where the majority of
                # pixels where not masked (QFperfect >=0.9) and likely stars
                # (rmeanpsfmag - rmeankronmag < 0.5)
                catalog = catalog[(catalog["ng"] > 3) *
                                  (catalog["nr"] > 3) *
                                  (catalog["ni"] > 3) *
                                  (catalog["gQfPerfect"] >= 0.95) *
                                  (catalog["rQfPerfect"] >= 0.95) *
                                  (catalog["iQfPerfect"] >= 0.95) *
                                  (catalog["rMeanPSFMag"] -
                                   catalog["rMeanKronMag"] < 0.5)]
