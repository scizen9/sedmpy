...
def solve_astrometry(img, outimage=None, radius=3, with_pix=True,
                     overwrite=True, tweak=3):
    """
    img: fits image where astrometry should be solved.
    outimage: name for the astrometry solved image. Default is "a_"img.
    radius: radius of uncertainty on astrometric position in image.
    with_pix: do we include the constraint on the pixel size for the RCCam?
    overwrite: replace previous solution?
    ...
    cmd = "solve-field --ra %s --dec %s --radius %.4f -p --new-fits %s" \
          " -W none -B none -P none -M none -R none -S none -t %d" \
          " --overwrite %s " % (ra, dec, radius, astro, tweak, img)
    if with_pix:
    ...
    if outimage and overwrite and os.path.isfile(astro):
        shutil.move(astro, outimage)
        return outimage
    elif not outimage and overwrite and os.path.isfile(astro):
...
def create_masterguide(lfiles, out=None):
    ...
    It will remove the bias from it, combine them using the median,
    and compute the astrometry for the image.
    ...
    iraf.imcombine(input="@"+bffile,
                   ...
                   combine="median",
                   scale="mode",
                   reject="sigclip", lsigma=2., hsigma=2,
                   gain=1.7, rdnoise=4.)
...
def __fill_ifu_dic(ifu_img, ifu_dic={}):
    """
    Fills some of the parameters for the IFU image that will be used to
    gather its guider images later.
    ...
    if imgtype == "SCIENCE" or imgtype == "STANDARD":
        ...
        # We only fill the dictionary if the exposure is
        # a valid science or standard image.
...
def __combine_guiders(ifu_dic, abspath, outdir):
    ...
    decs = np.array([cc.getDegDecString(fitsutils.get_par(r, "DEC"))
                     for r in rc])
    ...
    for ifu_i in ifu_dic.keys():
        ...
        mymask = (rcjd >= jd_ini) * (rcjd <= jd_end) * \
            (np.abs(ras - rad)*np.cos(np.deg2rad(decd)) < 0.5/60) * \
            (np.abs(decs - decd) < 0.5/60)
        guiders = rc[mymask]
        im = imtypes[mymask]
...
def make_guider(ifu_img, outdir):
    ...
    ifu_img: path to the IFU image we want to get the guiders for.
    """
    ifu_dic = {}
...
def make_guiders(ifu_dir, outdir):
    """
    Looks for all the IFU images in the directory and assembles all
    the guider images taken with RC for that time interval
    at around the coordinates of the IFU.

    ifu_dir: dir where IFU images are that we want to created the guiders for.
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--ifufile', type=str,
                        help='File containing the ifu image.', default=None)
    parser.add_argument('-d', '--outdir', type=str,
                        help='Directory containing the output stacked guider.',
                        default=None)
