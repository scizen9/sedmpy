...
def compute_offsets(fitsdir):
    ...
    for f in lfiles:
        ...
        try:
            ...
            if "[r]" in fitsutils.get_par(f, "OBJECT") or \
                    fitsutils.get_par(f, "OBJTYPE") == "ACQUISITION":
                ra = fitsutils.get_par(f, "OBJRA")
                dec = fitsutils.get_par(f, "OBJDEC")
                isAB = False
                if fitsutils.has_par(f, "ABPAIR") and \
                        fitsutils.get_par(f, "ABPAIR") == "True":
                    isAB = True
                if (ra != "" and dec != "" and
                    (ra != ra_prev or dec != dec_prev)) or \
                        (fitsutils.get_par(f, "OBJTYPE") == "ACQUISITION"):
                    # print ("Found image %s as first acquisition image after
                    # the slew. Computing offset for IFU..."%f)
                    recenter_ifu.main(os.path.abspath(f), isAB, astro=True,
                                      plot=True)
                ra_prev = ra
                dec_prev = dec
        except KeyError:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--directory', dest='directory', type=str,
                        help='Directory containing the fits for the night.')
