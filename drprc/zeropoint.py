...
try:
    ...
    import drprc.app_phot as app_phot
...
try:
    ...
    import drprc.QueryCatalogue as QueryCatalogue
try:
    import rcred
except ImportError:
...
def extract_star_sequence(imfile, band, plot=True, survey='ps1', debug=False,
    ...
    if (not np.any(mask) and not np.any(mask2) and
        ...
        logger.info('Average FWHM %.3f arcsec, %.3f pixels' %
                    (float(np.median(outd['fwhm'])),
                     float(np.median(outd['fwhm'])*pix2ang)))
        ...
        np.savetxt('/tmp/sdss_cat_det_%s.txt' %
                   creationdate, z[mask_valid_fwhm], fmt=fmt, header=header)
...
def add_to_zp_cal(ref_stars, image, logname):
    ...
    mask_valid1 = np.array(my["fwhm"] < 9000) * \
        np.array(~ np.isnan(r[band])) * np.array(~ np.isnan(my["fit_mag"])) * \
    ...
    logger.info("Adding %d stars to the ZP calibration file %s" % (nr, logname))
    ...
    if not os.path.isfile(logname):
        with open(logname, "a") as ff:
            ff.write("#object,filename,filter,std,stderr,inst,insterr,jd,"
...
def lsq_test():
    ...
    d = 23 + 2*xx + 4*yy + np.random.rand(100, 100)*1
    o = np.zeros_like(d)
    
    m = np.zeros((len(xx.flatten()), 3))
    m[:, 0] = 1
    m[:, 1] = xx.flatten()
    m[:, 2] = yy.flatten()
    ...
    ax1.plot(xx.flatten(), depend - coef[0] - yy.flatten()*coef[2], ".")
    ax1.plot(xx.flatten(), xx.flatten()*coef[1])
    ax1.set_xlabel("X")

    ax2.plot(yy.flatten(), depend - coef[0] - xx.flatten()*coef[1], ".")
    ax2.plot(yy.flatten(), yy.flatten()*coef[2])
    ax2.set_xlabel("Y")
...
def calibrate_zp_fourshot(logfile, plot=True):
    """
    The field of view is quite small, therefore, all rc shots are used
    to calibrate the zeropoint. This routine retrieves all the stars taken
    with the same filter when pointing to the science object.
    """
    ...
    plotdir = os.path.join(os.path.dirname(os.path.abspath(logfile)),
                           "zeropoint")

    for name in set(a['object']):    
        for b in set(a['filter']):
            aib = a[(a["filter"] == b) * (a["object"] == name) *
                    (np.abs(a["color"]) < 1)]
            ...
            coefs = np.polyfit(aib["std"], aib["inst"], 1,
                               w=1./np.maximum(0.3, np.sqrt(
                                   aib["stderr"]**2 + aib["insterr"]**2)))
            p = np.poly1d(coefs)
            
            if plot:
                plt.figure()
                plt.title("%s Filter %s" % (name, b))
                plt.errorbar(aib["std"], aib["inst"],
            ...
            if plot:
                ...
                plt.savefig(os.path.join(plotdir,
                                         "zp_mag_mag_%s_%s.png" % (name, b)))
                plt.close()

            # Then fit for the colour
            coefs = np.polyfit(aib["color"], aib["std"] - aib["inst"], 1,
                               w=1./np.maximum(0.15,
                                               np.sqrt(aib["stderr"]**2 +
                                                       aib["insterr"]**2)))
            p = np.poly1d(coefs)
            ...
            mad = stats.funcs.median_absolute_deviation(
                p(aib["color"]) - (aib["std"] - aib["inst"]))

            for ff in aib["filename"]:
                ...
                pardic = {"IQZEROPT": 1,
                          ...
                          "ZP": float("%.3f" % zp),
                          "ZPERR": float("%.3f" % mad)}
                fitsutils.update_pars(ff, pardic)

            if plot:
                plt.figure()
                plt.title("%s Filter %s" % (name, b))
                plt.errorbar(aib["color"],
                             aib["std"] - aib["inst"],
                             yerr=np.sqrt(aib["stderr"]**2 + aib["insterr"]**2),
                             fmt="o")
                plt.plot(aib["color"], p(aib["color"]))
                plt.savefig(os.path.join(plotdir,
                                         "zp_col_mag_%s_%s.png" % (name, b)))
                plt.close()
    return
...
def lsq_zeropoint(logfile, plotdir=".", plot=True):
    ...
    # Select only sources that have not been deviating too far from
    ...
    for b in ['g', 'r', 'i', 'u']:  # set(a['filter']):
        ...
        try:
            coefs = np.polyfit(
                ab["std"], ab["inst"],
                w=1./np.maximum(0.3,
                                np.sqrt(ab["stderr"]**2 + ab["insterr"]**2)),
                deg=1, full=True)
            p = np.poly1d(coefs)
            ...
            if plot:
                ...
                plt.errorbar(ab["std"], ab["inst"],
                             yerr=np.sqrt(ab["stderr"]**2 + ab["insterr"]**2),
                             fmt="bo", alpha=0.5, ms=2)
        ...
        lsq_result = np.linalg.lstsq(mm, depend)
        coef = lsq_result[0]

        # Empirical and predicted values
        est_zp = coef[0] + ab['color']*coef[1] + (ab['airmass']-1.3)*coef[2] + \
            coef[3]*ab['jd'] + coef[4]*ab['jd']**2 + coef[5]*ab['x'] + \
            coef[6]*ab['y'] + coef[7]*ab['x']**2 + coef[8]*ab['y']**2 + \
            coef[9]*(ab['fwhm']-1.5)  # + coef[5]*ab['jd']**3
        ...
        coefs_map = {'color': coef[1], 'airmass': coef[2], 'jd': coef[3],
                     'jd2': coef[4], 'jd3': coef[5], 'fwhm': coef[12],
                     'x': coef[6], 'y': coef[7], 'x2': coef[8], 'y2': coef[9],
                     'x3': coef[10], 'y3': coef[11]}
        # , 'x4':coef[12], 'y4':coef[13], 'fwhm': coef[14]}
        ...
        est_zp = coef[0] + coefs_map['color']*ab['color'] + \
            coefs_map['airmass']*(ab['airmass']-1.3) + \
            coefs_map['jd']*ab['jd'] + coefs_map['jd2']*ab['jd']**2 + \
            coefs_map['jd3']*ab['jd']**3 + coefs_map['x']*ab['x'] + \
            coefs_map['y']*ab['y'] + coefs_map['x2']*ab['x']**2 + \
            coefs_map['y2']*ab['y']**2 + coefs_map['x3']*ab['x']**3 + \
            coefs_map['y3']*ab['y']**3 + coefs_map['fwhm']*(ab['fwhm']-1.5)
        # + coef[5]*ab['jd']**3
        ...
        emp_jd = depend - (est_zp - (coefs_map['jd']*ab['jd'] +
                                     coefs_map['jd2']*ab['jd']**2 +
                                     coefs_map['jd3']*ab['jd']**3))
        pred_jd = coefs_map['jd']*ab['jd'] + coefs_map['jd2']*ab['jd']**2 + \
            coefs_map['jd3']*ab['jd']**3
        ...
        print("Filter %s ZP %.2f Color index %.2f RMS %.2f MAD %.2f" %
              (b, coef[0], coef[1], rms, mad))
        ...
        if plot:
            ...
            ax3.errorbar(ab['fwhm'],
                         depend - (est_zp - coefs_map['fwhm']*(ab['fwhm']-1.5)),
                         yerr=np.minimum(1, ab['insterr']), fmt="o", c=cols[b],
                         alpha=0.4, ms=4)
            # ax3.errorbar(ab['fwhm'], depend - est_zp,
            # yerr=np.minimum(1, ab['insterr']), fmt="o", c=cols[b],
            # alpha=0.4, ms=4)
            ax3.plot(ab['fwhm'], coefs_map['fwhm']*(ab['fwhm']-1.5),
                     color=cols[b])
            ax3.set_xlabel("FWHM")  
            ax3.set_title("FWHM")

            ax4.errorbar(ab['x'],
                         depend - (est_zp - (coefs_map['x']*ab['x'] +
                                             coefs_map['x2']*ab['x']**2 +
                                             coefs_map['x3']*ab['x']**3)),
                         yerr=np.minimum(1, ab['insterr']), fmt="o", c=cols[b],
                         alpha=0.4, ms=4)
            ...
            ax4.errorbar(ab['y'],
                         depend - (est_zp - (coefs_map['y']*ab['y'] +
                                             coefs_map['y2']*ab['y']**2 +
                                             coefs_map['y3']*ab['y']**3)),
                         yerr=np.minimum(1, ab['insterr']), fmt="o", c="blue",
                         alpha=0.4, ms=4)
            # ax4.plot(ab['x'], depend - est_zp, color=cols[b])
            ax4.set_xlabel("Y")  
            ...
            plt.errorbar(ab['std'], ab['std']-(ab['inst'] + est_zp),
                         yerr=np.sqrt(ab['insterr']**2 + ab['insterr']**2),
                         fmt="o", c=cols[b], alpha=0.3, ms=3)
            # plt.hist2d(ab['std'], ab['std']-(ab['inst'] + est_zp),
            # bins=(35,35), range=((13,20.5), (-0.2,0.2)),
            ...
            plt.hlines(0.05, xmin, xmax, linestyles="--", color="b",
                       linewidth=0.3)
            plt.hlines(-0.05, xmin, xmax, linestyles="--", color="b",
                       linewidth=0.3)
            plt.hlines(0.025, xmin, xmax, linestyles="--", color="red",
                       linewidth=0.3)
            plt.hlines(-0.025, xmin, xmax, linestyles="--", color="red",
                       linewidth=0.3)
            plt.ylim(-0.2, 0.2)
            ...
            diff = depend - est_zp
            im = ax1.scatter(ab['x'], ab['y'], c=depend - est_zp, s=4)
            # -(coefs_map['x']*ab['x'] + coefs_map['x2']*ab['x']**2 +
            # coefs_map['x3']*ab['x']**3 ) \
            # -(coefs_map['y']*ab['y'] + coefs_map['y2']*ab['y']**2 +
            # coefs_map['y3']*ab['y']**3) ))
            ...
            for i in range(1, len(myx)):
                diffsx.append(np.average(diff[(ab['x'] < myx[i]) *
                                              (ab['x'] > myx[i-1])]))
                diffsy.append(np.average(diff[(ab['x'] < myy[i]) *
                                              (ab['x'] > myy[i-1])]))
...
def interpolate_zp(reduced, logfile):
    ...
    zpfiles = [zf for zf in zpfiles if fitsutils.has_par(zf, "IQZEROPT") and
               (fitsutils.get_par(zf, "IQZEROPT") == 0 or
                fitsutils.get_par(zf, "ZEROPT") == 0 or
                fitsutils.get_par(zf, "ZPCAT") == "SDSSinterpolated")]
    ...
    for image in zpfiles:
        ...
        # Do not extrapolate outside of the valid interval.
        # jd = np.maximum(np.percentile(a['jd']-jdmin, 10),
        # fitsutils.get_par(image, "JD") - jdmin)
        jd = np.minimum(np.percentile(a['jd']-jdmin, 90),
                        fitsutils.get_par(image, "JD") - jdmin)
        airmass = fitsutils.get_par(image, "AIRMASS")
        x = a['x']
        y = a['y']
        ...
        if filt not in coefs:
            continue

        # est_zp = coef[0] +ab['color']*coef[1] +(ab['airmass']-1.3)*coef[2] +
        # coef[3]*ab['jd'] + coef[4]*ab['jd']**2 + coef[5]*ab['jd']**3 +
        # coef[6]*ab['jd']**4 + coef[7]*ab['jd']**5

        values = np.array([1, 0, airmass-1.3, jd, jd**2, jd**3, x, y,
                           x**2, y**2, x**3, y**3, fwhm-1.5])
...
def lsq_zeropoint_partial(logfile, plot=True):
    ...
    for b in set(a['filter']):
        ...
        print("Band", b, "zp %.2f, col %.2f" %
              (coef[0], coef[1]), "Residuals", res)
...
def find_zeropoint_noid(ref_stars, image, plot=True, plotdir="."):
    ...
    r = np.genfromtxt(ref_stars, delimiter=" ", dtype=None, names=True)
    imapp = os.path.join(os.path.join(os.path.dirname(image), "zeropoint"),
                         os.path.basename(image) + ".app.mag")

    my = np.genfromtxt(imapp, comments="#",
                       dtype=[("id", "<f4"),
                              ("image", "|S20"),
                              ("X", "<f4"),
                              ("Y", "<f4"),
                              ...
                              ("fit_mag", "<f4"),
                              ("fiterr", "<f4")])
    logger.info("Reading aperture magnitude from %s file. "
                "%d stars found here.\n The reference stars are in file %s, "
                "of length %d" % (imapp, len(my), ref_stars, len(r)))
    ...
    mask_valid1 = np.array(my["fwhm"] < 9000) * \
        np.array(my["fit_mag"] < 9000) * np.array(~ np.isnan(r[band])) * \
    ...
    if len(my) == 0:
        logger.warning("Warning, no reliable measurements for file %s. "
                       "Returning 0,0,0." % image)
        return 0, 0, 0
    ...
    coefs = np.polyfit(
        r[band]-r[col_band], r[band] - my["fit_mag"], 1,
        w=1./np.maximum(0.1, np.sqrt(my["fiterr"]**2 + r['d'+band]**2)))
    p = np.poly1d(coefs)
    
    logger.info("Coefficients for 1 deg polynomial fit to the zeropoint: ",
                coefs)
    ...
    if len(r) > 4:
        ...
        try:
            coefs = np.polyfit(
                r[band]-r[col_band], r[band] - my["fit_mag"], 1,
                w=1./np.maximum(0.2, np.sqrt(my["fiterr"]**2 + r['d'+band]**2)))
        except:
            logger.error("Critical failure for polyfit for object %s" % image)
            return 0, 0, 0
        
        logger.info("Coefficients for 1 deg polynomial fit to the zeropoint "
                    "after outlier rejection:", coefs)
        p = np.poly1d(coefs)
    ...
    if plot:
        plt.errorbar(r[band]-r[col_band], r[band] - my["fit_mag"],
                     yerr=np.sqrt(my["fiterr"]**2 + r['d'+band]**2),
                     marker="o", ls="None")
        for i, myid in enumerate(ids):
            plt.text(r[band][i]-r[col_band][i] + 0.01,
                     r[band][i] - my["fit_mag"][i] + 0.01, str(myid))
        x = np.linspace(np.min(r[band]-r[col_band]),
                        np.max(r[band]-r[col_band]), 100)
        plt.plot(x, p(x))
        plt.title("Best fit ZP: %.2f colour term: %.2f MAD: %.2f" %
                  (p[0], p[1], mad))
        plt.xlabel("{:} - {:}".format(band, col_band))
        plt.ylabel("ZP")
        plotname = os.path.join(
            plotdir, os.path.basename(image).replace(
                '.fits', ".zp.png").replace('.new', ".zp.png"))
        plt.tight_layout()
...
def calibrate_zeropoint(image, plot=True, plotdir=None, astro=False,
                        debug=False, refstars=None, minexp=1):
    """
    ...
    logger.info("Starting calibration of ZP for image %s for "
                "object %s with filter %s." % (image, objname, band))

    if exptime < minexp:
        logger.error("ERROR. Exposure time too short for image (%s) "
                     "to see anything..." % image)
        return 

    # repeat astrometry
    if astro:
        rcred.solve_astrometry(image, radius=3, with_pix=True, overwrite=True,
                               tweak=3)

    if filt == 'u':
        calcat = "SDSS"
        extracted = extract_star_sequence(os.path.abspath(image), filt,
                                          plot=plot, survey='sdss', debug=debug,
                                          refstars=refstars, plotdir=plotdir)
    else:
        calcat = "PS1"
        extracted = extract_star_sequence(os.path.abspath(image), filt,
                                          plot=plot, survey='ps1', debug=debug,
                                          refstars=refstars, plotdir=plotdir)
        
    if not extracted:
        logger.warning("Field+filter not in SDSS or error when retrieving the "
                       "catalogue... Skipping. "
                       "Image %s not zeropoint calibrated." % image)
        # Add these values to the header.
        pardic = {"IQZEROPT": 0, "ZPCAT": "None", "ZEROPTU": 0, "ZEROPT": 0,
                  "ZP": 0, "ZPERR": 0, "%s_coef" % filt: 0}
        fitsutils.update_pars(image, pardic)
        return 
    ...
    app_phot.get_app_phot("/tmp/sdss_cat_det_%s.txt" % creationdate, image,
                          wcsin='logic', plotdir=plotdir, box=15)
    
    # Compute the zeropoint for the specific image.
    z, c, err = find_zeropoint_noid("/tmp/sdss_cat_det_%s.txt" % creationdate,
                                    image, plot=plot, plotdir=plotdir)
    
    # Add these values to the header.
    pardic = {"IQZEROPT": 1, "ZPCAT": calcat, "ZEROPTU": np.round(err, 3),
              "ZEROPT": np.round(z, 3), "%s_coef" % filt: c}
    fitsutils.update_pars(image, pardic)
    ...
    if not os.path.isfile(logname):
        with open(logname, "a") as ff:
            ff.write("#filename,exptime,filter,date,airmass,fwhm_pix,"
                     "fwhm_as,zeropoint,color,err\n")
    with open(logname, "a") as ff:
        ff.write("%s,%.1f,%s,%3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n" %
                 (image, exptime, filt, date, airmass, fwhm_pix, fwhm, z, c,
                  err))
...
def plot_zp(zpfile, plotdir=None):
    ...
    # We add 5h to the UTC date, so it alwasy keeps the
    # date of the end of the night.
    ...
    for fi in ['u', 'g', 'r', 'i']:
        ...
        for i in range(len(a[a['filter'] == fi])):
            ...
            plt.errorbar(datestat, a[a['filter'] == fi]['zeropoint'][i],
                         yerr=a[a['filter'] == fi]['err'][i], marker='o',
                         mfc=cols[fi], mec='k', ecolor=cols[fi], ls='none',
                         ms=a[a['filter'] == fi]['fwhm_pix'][i]*2)
        logger.info("Median zeropoint for filter %s: %.2f mag" %
                    (fi, np.median(a[a['filter'] == fi]['zeropoint'])))
...
def plot_zp_airmass(zpfile):
    ...
    for fi in ['u', 'g', 'r', 'i']:
        for i in range(len(a[a['filter'] == fi])):
            plt.errorbar(a[a['filter'] == fi]['airmass'][i],
                         a[a['filter'] == fi]['zeropoint'][i],
                         yerr=a[a['filter'] == fi]['err'][i],
                         marker='o', mfc=cols[fi], mec='k', ecolor=cols[fi],
                         ls='none', ms=a[a['filter'] == fi]['fwhm_pix'][i])
        logger.info("Median zeropoint for filter %s: %.2f mag" %
                    (fi, np.median(a[a['filter'] == fi]['zeropoint'])))
...
def reset_zp(directory):
    ...
    for fl in files:
        if fitsutils.get_par(fl, "ZPCAT") == "SDSSinterpolated":
            fitsutils.update_par(fl, "IQZEROPT", 0)
            fitsutils.update_par(fl, "ZEROPT", 0)
            fitsutils.update_par(fl, "ZEROPTU", 0)
...
def main(reduced):
    ...
    for ff in glob.glob("*_a_*.fits"):
        logger.info("Starting calibration of zeropoint for %s" % f)
        if not fitsutils.has_par(ff, "IMGTYPE") or \
                fitsutils.get_par(ff, "IMGTYPE").upper() == "SCIENCE":
            calibrate_zeropoint(ff, plotdir=plotdir)
