...
def dbconnect():
    ...
    try:
        db = pg.DB(host='localhost', dbname='ptftransient', user='ptftransient',
                   passwd='discovery')
...
def upload_sedm(sedmfile, db):
    ...
    """ id | sourceid | programid | instrumentid |         obsdate
            | exptime | snr | snrwav | minwav  | maxwav  |
    dataformat |                   datapath                   |      observer
           | reducedby | classification |     lastmodified        | spectype |
            uploadedby """
    ...
    params = {
        ...
        'programid': 4, 'instrumentid': 65, 'obsdate': sedmp["OBSUTC"][0:8],
        'exptime': sedmp["EXPTIME"], 'snr': '', 'snrwav': '',
        'minwav': np.nanmin(spec[:, 0]), 'maxwav': np.nanmax(spec[:, 0]),
        'dataformat': 'ascii',
        'datapath': os.path.join('ptf/spectra/data/',
                                 os.path.basename(sedmfile)),
        'observer': 'SEDM robot', 'reducedby': 'SEDM reducer',
        'spectype': 'object', 'uploadedby': 'bookkeeper'
    }

    if "SNIDMATCHRLAP" in sedmp and float(sedmp["SNIDMATCHRLAP"]) > 5:
...
def upload_snid_plot(sedmfile, plotfile, db):
    """ commit the classification comments
       ...
       id   | tablename | tableid | username |  type   | comment | date
          | sourceid |   attachment
    """
    # Retrieve information from the SEDM file header.
    sedmp = get_sedm_paramters(sedmfile)
    ...
    if "SNIDMATCHRLAP" in sedmp and float(sedmp["SNIDMATCHRLAP"]) > 5:
        comment = "SNID automatic classification. " \
                  "Best fit is a %s %s at z=%.3f+-%.3f at %d+-%d days" % \
    ...
    # Fill the parameters for the DB with the right values.
    params = {
        'id': -1, 'tablename': 'sources', 'tableid': sourceid,
        'username': 'bookkeeper', 'type': 'info', 'comment': comment,
        'sourceid': sourceid,
        'attachment': os.path.join('attachments/', os.path.basename(plotfile))
    }
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        ...
        copies the sedm file to the yupana final destination and 
        updates the database.
            
        """, formatter_class=argparse.RawTextHelpFormatter)

    parser.add_argument('-f', '--sedmfile', type=str, dest="specfile",
                        help='SEDMfile in ascii format.', default=None)
    parser.add_argument('-c', '--copytodest', action='store_true', dest="copy",
                        default=True,
                        help='Whether to copy this file to its marshal'
                             ' destination.')
    parser.add_argument('-o', '--overwrite', action='store_true',
                        dest="overwrite", default=False,
                        help='Whether to overwrite the current file with'
                             ' a newer version.')
    ...
    plotfile = glob.glob(os.path.join(os.path.dirname(sedmfile),
                                      "*"+name+"*png"))
    ...
    # True by default.
    if copy:
        sedmfiledest = os.path.join(
            "/scr/apache/htdocs/marshals/transient/ptf/spectra/data",
            os.path.basename(sedmfile))
        
        # If the file does not exist in destination, we copy it
        if not os.path.isfile(sedmfiledest):
            print("File %s does not exist in destination as %s. "
                  "Moving it there and updating the marshal."
                  % (sedmfile, sedmfiledest))
        ...
        # If the file does exist
            ...
            if overwrite:
                ...
                match = re.search(r'_v\(?([0-9]+)\)?',
                                  os.path.basename(sedmfile))
                version = int(match.group(1))
                sedmfiledest = os.path.join(
                    os.path.dirname(sedmfiledest),
                    os.path.basename(sedmfile).replace("_v%d." % version,
                                                       "_v%d." % (version+1)))
                print("Moving file %s to %s" % (sedmfile, sedmfiledest))
                shutil.move(sedmfile, sedmfiledest)
            # If the file already exists and we don't overwrite,
            # we just delete it from the reception folter.
            # Generally this means that the spectrum is already in the marshal.
            else:
                print("File %s already exists in destination as %s."
                      " No overwrite selected. Skipping."
                      % (sedmfile, sedmfiledest))
                os.remove(sedmfile)
            if copyplotfile:
                os.remove(plotfile)
    ...
    # True by default.
    if copyplotfile:
        sedmplotfiledest = os.path.join(
            "/scr/apache/htdocs/marshals/transient/ptf/attachments",
