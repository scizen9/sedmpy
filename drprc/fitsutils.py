...
if __name__ == '__main__':
    ...
    parser.add_option("-d", "--dir", action="store", type="string", dest="dir",
                      metavar="Directory to be plotted", default=".")
    parser.add_option("-a", "--ra", action="store", type="float", dest="ra",
                      metavar="ra", default=0.0)
    parser.add_option("-b", "--dec", action="store", type="float", dest="dec",
                      metavar="dec", default=0.0)
    parser.add_option("-p", "--pattern", action="store", type="string",
                      dest="pattern", metavar="pattern", default="*.fits")
    ...
    for f in glob.glob(mydir + "/" + pattern):
        ...
        for i in indices:
            ...
            im = plt.imshow(plt.log10(img), aspect="equal",
                            extent=(0, ny, 0, nx),
                            origin="lower", cmap=matplotlib.cm.gray_r,
...
def move_to_discarded(mydir, myfilter, ra, dec):
    ...
    for f in glob.glob(os.path.join(mydir, myfilter)):
        ...
        if len(frames_with_target) == 0:
            ...
            extract_field_from_moscaic(mydir, os.path.basename(f),
                                       frames_with_target, origname=True)
...
def get_frames_with_target(myfile, ra, dec, debug=False):
    ...
    for i in indices:
        ...
        if ra * dec != 0:
            ...
            wcs = WCS(prihdr)
            try:
                target_pix = wcs.wcs_sky2pix([(np.array([ra, dec],
                                                        np.float_))], 1)[0]
            except:
                print("ERROR when converting sky to wcs. Is astrometry in "
                      "place? Default coordinates assigned.")
        ...
        if (0 < target_pix[0] < nx) and (0 < target_pix[1] < ny):
            frames.append(i)
...
def cut_frame_with_target(myfile, ra, dec, h=4000, w=2000, debug=False):
    ...
    if ra * dec != 0:
        ...
        try:
            ...
            print("ERROR when converting sky to wcs. Is astrometry in place? "
                  "Default coordinates assigned.")
            target_pix = [+nx/2., ny/2.]
    ...
    for i, f in enumerate(glob.glob(mydir + "/" + myfilter)):
        ...
        for n in nfields:
            ...
            if origname:
                ...
                hdulist1.writeto("out%d_%d.fits"%(i,n), output_verify="fix",
                                 clobber=True)
        
        
def unify_header(prihdr):
    """
    Reads the different fields from different telescopes and unifies them
    under common names.

    """
    
    dic = {"GAIN": 0, "RDNOISE": 0, "AIRMASS": 0, "EXPTIME": 0,
           "FILTER": 0, "MJD-OBS": 0}
...
def unify_fits(myfits, overwrite=False, field=-1):
    """
    Creates a unified standard version of the fits file header, creating
    standard fields for the most used fields.
    """
    ...
    if nfields > 1:
        if field == -1 and datafields>1:
            print("WARNING, there are several images stored. Plsease, specify "
                  "which number you want to extract.")
            print("Exiting...")
            return 
...
def arrange_fits_in_directories(mydir, myfilter, destdir):
    """
    Automatically sorts the fits files ina directory structure with the
    filter name as the fierst level, and the integer of the MJD of the
    observation at the second level.
    ...
    for f in glob.glob(os.path.join(mydir, myfilter)):
        ...
        destination = os.path.join(destdir, "%s/%s/%d" % (instrument,
                                                          filt, mjd))
...
def get_gain_ron_combined_i(gain, ron, n, mode="median"):
    ...
    if mode == "sum":
            ...
            print("Unknown selected mode. Avaialable values: "
                  "(sum, average, median)")
