...
hits on images (numpy arrays or FITS), using scipy, and based on
Pieter van Dokkum's L.A.Cosmic algorithm.
...
I pimped this a bit to suit my needs :
    ...
    Indeed saturated stars tend to get even uglier when you try to clean them.
    Plus they keep L.A.Cosmic iterations going on forever.
    This feature is mainly for pretty-image production. It is optional,
    requires one more parameter (a CCD saturation level in ADU), and uses some
    nicely robust morphology operations and object extraction.

    - Scipy image analysis allows to "label" the actual cosmic ray hits
    (i.e. group the pixels into local islands).
    A bit special, but I use this in the scope of visualizing
    a PSF construction.

But otherwise the core is really a 1-to-1 implementation of L.A.Cosmic, and
uses the same parameters.
Only the conventions on how filters are applied at the image edges might be
different.
...
You need astropy.io.fits, numpy and scipy.
See the demo scripts for example usages (the second demo uses f2n.py to
make pngs, and thus also needs PIL).
...
class CosmicsImage:
    ...
    def __init__(self, rawarray, pssl=0.0, gain=2.2, readnoise=10.0,
        ...
        Constructor of the cosmic class, takes a 2D numpy array of your image
        as main argument.
        ...
        consider it to be a saturated star and
        do not try to correct and pixels around it. A negative satlevel skips
        this feature.
        ...
        real   sigfrac = 0.5          # fractional detection limit for
                                                neighbouring pixels
        real   objlim  = 3.0           # contrast limit between CR and
                                                underlying object
    ...
    def __str__(self):
        ...
        """
        stringlist = ["Input array : (%i, %i), %s" % (self.rawarray.shape[0],
        ...
        if self.satstars is not None:
            stringlist.append("Saturated star mask : %i pixels" %
                              np.sum(self.satstars))
    ...
    def labelmask(self, verbose=None):
        ...
        centers = [[(tup[0].start + tup[0].stop) / 2.0,
                    (tup[1].start + tup[1].stop) / 2.0]
                   for tup in slicecouplelist]
        # We also want to know how many pixels where affected
        # by each cosmic ray.
    ...
    def clean(self, mask=None, verbose=None):
        ...
        but it's a bit harder to do in python, as there is no readymade masked
        median. So for now we do a loop...
        ...
        self.mask, you can supply your own mask as argument. This might be
        useful to apply this cleaning function iteratively. But for the true
        ...
        # that copy is important,
        padarray[2:w + 2, 2:h + 2] = self.cleanarray.copy()
        # we need 2 independent arrays
        ...
        for cosmicpos in cosmicindices:
            ...
            if np.alen(goodcutout) >= 25:
                ...
                # i.e. no good pixels : Shit, a huge cosmic,
    ...
    def findsatstars(self, verbose=None):
        ...
        these regions in cosmic detection and cleaning procedures.
        Slow ...
        """
        if verbose is None:
            verbose = self.verbose
    ...
    def lacosmiciteration(self, verbose=None):
        ...
        adding the newly detected
        cosmics to the existing self.mask. Cleaning is not made automatically !
        You have to call
        ...
        # cliped = np.abs(conved) # unfortunately this does not work to
        # find holes as well ...
        ...
        return {
            "niter": nbfinal, "nnew": nbnew, "itermask": finalsel,
            "newmask": newmask
        }
    ...
    def run(self, maxiter=4, verbose=False):
        ...
        for i in range(1, maxiter + 1):
            ...
            print("%i cosmic pixels (%i new)" % (iterres["niter"],
                                                 iterres["nnew"]))
...
def fromfits(infilename, hdu=0, verbose=True):
    ...
    pixelarray, hdr = pf.getdata(infilename, hdu, header=True)
    pixelarray = np.asarray(pixelarray).transpose()

    pixelarrayshape = pixelarray.shape
    if verbose:
        print("FITS import shape : (%i, %i)" % (pixelarrayshape[0],
                                                pixelarrayshape[1]))
...
def tofits(outfilename, pixelarray, hdr=None, verbose=True):
    ...
    If you specify a header (fits format, as returned by fromfits())
    it will be used for the image.
    You can give me boolean numpy arrays, I will convert them into
    8 bit integers.
    """
    pixelarrayshape = pixelarray.shape
    if verbose:
        print("FITS export shape : (%i, %i)" % (pixelarrayshape[0],
                                                pixelarrayshape[1]))
    ...
    if hdr is None:  # then a minimal header will be created
        hdu = pf.PrimaryHDU(pixelarray.transpose())
    else:  # this if else is probably not needed but anyway ...
        hdu = pf.PrimaryHDU(pixelarray.transpose(), hdr)
...
def subsample(a):  # this is more a generic function then a method ...
    """
    Returns a 2x2-subsampled version of array a (no interpolation,
    just cutting pixels in 4).
