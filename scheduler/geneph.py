...
def get_observing_times(obsdatetime=None, return_type=''):
    ...
    ret = {'sun_set':
               obs_site.sun_set_time(obstime,
                                          which=which),
           'sun_rise':
               obs_site.sun_rise_time(obstime,
                                           which=which),
           'evening_civil':
               obs_site.twilight_evening_civil(obstime,
                                                    which=which),
           'evening_nautical':
               obs_site.twilight_evening_nautical(obstime,
                                                       which=which),
           'evening_astronomical':
               obs_site.twilight_evening_astronomical(obstime,
                                                           which=which),
           'morning_civil':
               obs_site.twilight_morning_civil(obstime,
                                                    which=which),
           'morning_nautical':
               obs_site.twilight_morning_nautical(obstime,
                                                       which=which),
           'morning_astronomical':
               obs_site.twilight_morning_astronomical(obstime,
                                                           which=which)}
...
def get_ephem(target, utdate, use_visible_window=True, return_nearest=True,
    ...
    for i, _ in enumerate(horizons['V']):
        if horizons['solar_presence'][i] == True or horizons['EL'][i] < 12 or horizons['airmass'][i] > 2.3:
        ...
        if print_all:
            print('%10s %10f %10f %07f %10f %10f %s %06f' % (
                target, horizons['RA'][i], horizons['DEC'][i], 2000.0, horizons['RA_rate'][i],
                horizons['DEC_rate'][i],
                horizons['datetime_str'][i], horizons['V'][i]))

    if nearest != 1000 and index != -1:
        print('%10s %10f %10f %07f %10f %10f %s %06f' % (
            target, horizons['RA'][index], horizons['DEC'][index], 2000.0, horizons['RA_rate'][index],
...
def get_ephem2(target, utdate):
    ...
    for i, _ in enumerate(horizons['V']):
        ...
        print('%10s %10f %10f %07f %10f %10f %10f %06f' % (
            target, horizons['RA'][i] / 24, horizons['DEC'][i], 2000.0, horizons['RA_rate'][i],
            horizons['DEC_rate'][i],
            (horizons['datetime_jd'][i] - np.floor(horizons['datetime_jd'][i])) * 24, horizons['V'][i]))


def command_line():
    t = Time('%s-%s-%sT00:00:00' % (sys.argv[2][0:4], sys.argv[2][4:6], sys.argv[2][6:8]), format='isot', scale='utc')
    ...
    print('%10s %10s %10s %07s %10s %10s %10s %06s' % ('name', 'RA', 'Dec', 'equinox', 'RA rate', 'Dec rate', 'time', 'V'))

    for i, _ in enumerate(horizons['V']):
        if horizons['solar_presence'][i] == True or horizons['EL'][i] < 0:
            continue

        print('%10s %10f %10f %07f %10f %10f %10f %06f' % (
        sys.argv[1], horizons['RA'][i] / 24, horizons['DEC'][i], 2000.0, horizons['RA_rate'][i], horizons['DEC_rate'][i],
        (horizons['datetime_jd'][i] - np.floor(horizons['datetime_jd'][i])) * 24, horizons['V'][i]))
